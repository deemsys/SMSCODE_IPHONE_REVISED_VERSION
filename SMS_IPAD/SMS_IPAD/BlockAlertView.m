...
+ (void)initialize
{
    if (self == [BlockAlertView class])
    {
        ...
        if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
             background = [UIImage imageNamed:@"alert_bg.png"];
        } else {
             background = [UIImage imageNamed:@"alert_bg@2x.png"];
        }
...
- (void)show
{
    ...
    for (NSUInteger i = 0; i < _blocks.count; i++)
    {
     ...
     //  image= [image stretchableImageWithLeftCapWidth: 25 topCapHeight: 40];
        ...
        if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
          ...
          button.frame = CGRectMake(110, _height,30, 20);
        } else
        {
          button.frame = CGRectMake(110, _height, 30, 30);
        }
