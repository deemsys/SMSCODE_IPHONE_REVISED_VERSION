...
- (void)keyboardWillShow:(NSNotification*)aNotification
{
    ...
    CGFloat yTexfieldPosition = [self->activeEditField convertPoint:CGPointMake(0, 0) toView:self.view].y + self->activeEditField.frame.size.height+10;
    // adjustements relative to statusBar visibility
    if(![UIApplication sharedApplication].statusBarHidden)
    {
        if ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeLeft || [UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeRight)
        {
            yTexfieldPosition += [UIApplication sharedApplication].statusBarFrame.size.width+30;
        }
        else
        {
            yTexfieldPosition += [UIApplication sharedApplication].statusBarFrame.size.height+30;
        }
    }
